...
describe("DeleteItemAbl Tests", () => {
  describe("Happy Path (Positive Scenarios)", () => {
    // Test for successfully deleting an item from a shopping list
    it("should delete an item from a shopping list", async () => {
      ...
      const req = {
        body: { listId: "674b3f899ea7a953682fb779", itemId },
      ...
      expect(res.json).toHaveBeenCalledWith({
        status: "success",
        message: `Item ${itemId} deleted from shopping list 674b3f899ea7a953682fb779`,
  ...
  describe("Alternative Scenarios (Error Handling)", () => {
    ...
    it("should return 400 if listId is missing", async () => {
      const req = {
        body: { itemId: crypto.randomBytes(16).toString("hex") },
        headers: { "x-user-id": new ObjectId().toString() },
        sanitize: (value) => value,
        db,
      };
